...
public class Logic03Soal0301 {
	...
	// method tambahan untuk menampilkan deret angka
	static int[] deretTiga(int n, int m, int o) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (i % (m + 1) == m) {
				array[i] = m; // jika hasil bagi antara nilai i dengan m+1 itu bernilai 3, maka cetak ini
			} else {
				array[i] = nilaiAwal;
